    ...
    <Link href={href} className="relative group w-full block outline-none">
      ...
      <div ref={ref} className={`relative flex items-center ${isCollapsed ? 'justify-center px-0 py-3' : isChild ? 'pl-10 pr-2 py-1.5' : 'px-3 py-2.5'} transition-all duration-300`}>
        ...
        <motion.div
          ...
          className={`relative z-10 p-2 rounded-xl transition-colors duration-200 ${isActive ? 'bg-[#008080] text-white shadow-lg shadow-teal-500/30' : 'text-gray-500 group-hover:text-[#008080] group-hover:bg-white/50 dark:group-hover:bg-white/10'
            }`}
        >
  ...
  return (
    <>
      ...
      <UpgradeOverlay
        ...
        onSuccess={() => { }}
        originPosition={upgradeButtonPosition}
      />
    </>
  );
}