...
export const defaultMetallicPaintParams: MetallicPaintParams = {
  patternScale: 2,
  refraction: 0.015,
  edge: 1,
  patternBlur: 0.005,
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = function () {
      try {
        ...
        // Ensure a white background so the mask logic works reliably
        ...
        const shapeMask = new Array<boolean>(width * height).fill(false);
        ...
        const boundaryMask = new Array<boolean>(width * height).fill(false);
        for (let y = 0; y < height; y++) {
          for (let x = 0; x < width; x++) {
            const idx = y * width + x;
        ...
        reject(err instanceof Error ? err : new Error('Failed to parse image'));
      } finally {
...
function getUniformLocations(gl: WebGL2RenderingContext, program: WebGLProgram) {
...
export default function MetallicPaint({
  ...
  const mergedParams = useMemo<MetallicPaintParams>(() => {
    return { ...defaultMetallicPaintParams, ...(params ?? {}) };
  }, [params]);

  const activeImageData = propImageData ?? generatedImageData;

  // Parse SVG markup (preferred)
  useEffect(() => {
    if (!svg) return;

    const blob = new Blob([svg], { type: 'image/svg+xml' });
    const file = new File([blob], 'art.svg', { type: 'image/svg+xml' });

    let cancelled = false;
    parseLogoImage(file)
      .then((result) => {
        if (!cancelled) setGeneratedImageData(result.imageData);
      })
      .catch((err) => {
        // eslint-disable-next-line no-console
  ...
  useEffect(() => {
    ...
    const file = new File([blob], 'path.svg', { type: 'image/svg+xml' });
  ...
  // Resize canvas to element size + set ratios
  useEffect(() => {
    const canvas = canvasRef.current;
    const gl = glRef.current;
    const uniforms = uniformsRef.current;
    if (!canvas || !gl || !activeImageData) return;
    ...
    const resize = () => {
      ...
      const imgRatio = activeImageData.width / activeImageData.height;
  ...
  // Upload texture whenever image data changes
  useEffect(() => {
    const gl = glRef.current;
    const uniforms = uniformsRef.current;
    if (!gl || !activeImageData) return;
